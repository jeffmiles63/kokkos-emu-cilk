...
class ParallelFor<FunctorType, Kokkos::RangePolicy<Traits...>,
 ...
 private:
  ...
  typename std::enable_if<std::is_same<TagType, void>::value>::type inner_exec(
      int inLen_, int sc_, int iLoop, int start) const {
    if (sc_ > 1) {
      for (int s = 0; s < sc_; s++) {
        // printf("spawn inner: s = %d , inlen = %d, len = %d \n", (const int)s,
        // (const int)inLen_, (const int)iLoop * sc_ + s); fflush(stdout);
        _Cilk_spawn inner_exec<TagType>(inLen_, 0, iLoop * sc_ + s, start);
      }
      cilk_sync;
    } else {
      // printf("inner: inlen = %d, len = %d \n", (const int)inLen_, (const
      // int)iLoop); fflush(stdout);
  ...
  template <class TagType>
  typename std::enable_if<!std::is_same<TagType, void>::value>::type inner_exec(
      const TagType t, int inLen_, int sc_, int iLoop, int start) const {
    if (sc_ > 1) {
      for (int s = 0; s < sc_; s++) {
        // printf("spawn inner: s = %d , inlen = %d, len = %d \n", (const int)s,
        // (const int)inLen_, (const int)iLoop * sc_ + s);
      ...
      for (int j = 0; j < inLen_; j++) {
        int ndx = start + offset + j;
        if (ndx < m_policy.end()) m_functor(t, ndx);
      }
    }
  }
  ...
  typename std::enable_if<std::is_same<TagType, void>::value>::type exec()
    ...
    const typename Policy::member_type par_loop =
        len > MAX_THREAD_COUNT ? MAX_THREAD_COUNT : len;
    ...
    cilk_for(typename Policy::member_type i = 0; i < par_loop; ++i) {
      for (typename Policy::member_type j = (int_loop * i);
           j < ((int_loop * i) + int_loop); j++) {
        if (j < e) {
    ...
    // printf(" tree spawn parallel for: b= %d, e = %d, l = %d, par = %d, sc =
    // %d, int = %d \n", b, e, len, par_loop, sc_count, int_loop);
    // fflush(stdout);
    for (typename Policy::member_type i = 0; i < mz;
         ++i) {  // This should be the number of nodes...
      // printf(" parallel for spawn: i = %d , %08x \n", (const int)i,
      // &refPtr[i]); fflush(stdout); _Cilk_migrate_hint(&refPtr[i]);
  ...
  typename std::enable_if<!std::is_same<TagType, void>::value>::type exec()
    ...
    int sc_count = (par_loop / mz) + (((par_loop % mz) == 0) ? 0 : 1);
    if (sc_count == 0) sc_count = 1;
    // printf("T tree spawn parallel for: b= %d, e = %d, l = %d, par = %d, sc =
    // %d, int = %d \n", b, e, len, par_loop, sc_count, int_loop);
...
class ParallelReduce<FunctorType, Kokkos::RangePolicy<Traits...>, ReducerType,
 ...
 private:
  ...
  typedef
      typename cilk_reducer_wrapper::ReducerTypeFwd::value_type reduction_type;
  ...
  const typename Policy::member_type get_policy_int_loop() const {
    if (m_policy_par_loop > 0)
      return ((m_policy_len / m_policy_par_loop) +
              (((m_policy_len % m_policy_par_loop) == 0) ? 0 : 1));
    else
      return 1;
  }
  ...
  const typename Policy::member_type get_policy_par_size() const {
    if (m_policy_par_loop > 0)
      return ((m_policy_par_loop /
               Kokkos::Experimental::EmuReplicatedSpace::memory_zones()) +
              (((m_policy_par_loop %
                 Kokkos::Experimental::EmuReplicatedSpace::memory_zones()) == 0)
                   ? 0
                   : 1));
    else
      return 1;
  ...
  internal_reduce(const typename Policy::member_type b,
                  const typename Policy::member_type e, int i,
                  const size_t l_alloc_size, int nl_) const {
    int array_ndx  = i % nl_;
    int reduce_off = i / nl_;
    // printf("[%d] reduce ndx = %d, off = %d \n", i, array_ndx, reduce_off);
    // fflush(stdout);
    cilk_reducer_wrapper* pReducer =
        get_reducer<cilk_reducer_wrapper>(global_reducer, array_ndx);
    // printf("void [%d.%d] internal reduce: %lx, %d, %d, %d, %d \n", NODE_ID(),
    // array_ndx, (unsigned long)pReducer, e, m_policy_par_size, i,
    // m_policy_int_loop); Kokkos::Experimental::print_pointer(i, pReducer,
    // "reducer pointer" );
    ...
    pointer_type pRef = (pointer_type)mw_arrayindex(
        (void*)working_ptr, array_ndx, nl_, l_alloc_size);
    // Kokkos::Experimental::print_pointer(i, pRef, "internal reduce (outer)" );
    // Kokkos::Experimental::print_pointer(i, &pRef[i%par_size], "internal
    // reduce (inner)" );
    //     printf("[%d] obtaining update reference %d: %lx, offset %d\n", i,
    //     array_ndx, pRef, i%par_size); fflush(stdout);
    // reference_type lupdate = ValueInit::init(  pReducer->f ,
    // &pRef[i%par_size] ); pRef[i%par_size] = reduction_type();
    reduction_type& lupdate = (reduction_type&)*(&pRef[reduce_off]);
    // printf("[%d.%d] pointer node: %d \n", NODE_ID(), THREAD_ID(),
    // mw_ptrtonodelet(&lupdate) ); Kokkos::Experimental::print_pointer(i,
    // &lupdate, "entering inner loop" );

    for (typename Policy::member_type j = (b + (m_policy_int_loop * i));
         j < (b + ((m_policy_int_loop * i) + m_policy_int_loop)); j++) {
      if (j < e) {
        pReducer->init(lupdate);
        pReducer->f((const typename Policy::member_type)j, lupdate);
        // printf("[%d, %d] return from functor: %d - %d (%lx) \n", array_ndx,
        // reduce_off, (int)j, lupdate, &lupdate); fflush(stdout);
  ...
  internal_reduce(const typename Policy::member_type b,
    ...
    // (unsigned long)pReducer, b, m_policy_par_size, i);
    ...
    // Kokkos::Experimental::print_pointer(i, pRef, "internal reduce (outer)" );
    // Kokkos::Experimental::print_pointer(i, &pRef[i%par_size], "internal
    // reduce (inner)" ); printf("[%d] obtaining update reference %d, %d: %08x,
    // offset %d\n", NODE_ID(), i, array_ndx, pRef, i%par_size); reference_type
    // lupdate = ValueInit::init(  pReducer->f , &pRef[i%par_size] );
  ...
  void init_reducer(const size_t l_alloc_bytes, int i) const {
    cilk_reducer_wrapper* pH =
        get_reducer<cilk_reducer_wrapper>(global_reducer, i);
    typename cilk_reducer_wrapper::local_reducer_type* pLocalRed =
        get_reducer<typename cilk_reducer_wrapper::local_reducer_type>(
            local_reducer, i);
    //	 printf("init reducer: %lx, %lx \n", (unsigned long)pH, (unsigned
    //long)pLocalRed); 	 fflush(stdout);
    new (pLocalRed)
        NodeletReducer<typename cilk_reducer_wrapper::reduce_container>(
            global_reducer, i, cilk_reducer_wrapper::default_value());
    new (pH) cilk_reducer_wrapper(
        global_reducer, i, ReducerConditional::select(m_functor, m_reducer),
        l_alloc_bytes, pLocalRed);
  }

  void initialize_cilk_reducer(const size_t l_alloc_bytes) const {
    long* refPtr = Kokkos::Experimental::EmuReplicatedSpace::getRefAddr();
    // printf("reducer init: %lx, %lx, %lx \n", global_reducer, local_reducer,
    // working_ptr); fflush(stdout);
    for (int i = 0;
         i < Kokkos::Experimental::EmuReplicatedSpace::memory_zones(); i++) {
  ...
  inline typename std::enable_if<std::is_same<TagType, void>::value>::type exec(
    ...
    //      %lx\n", b, e, len, par_loop, int_loop, w_ptr); cilk_for (typename
    //      Policy::member_type i = 0 ; i < par_loop ; ++i ) {
    //      }

    // printf("internal reduce size = %d ... spawning threads... \n",
    // (int)l_alloc_bytes); fflush(stdout);
    for (int i = 0; i < m_policy_par_loop; i++) {
      int node_ = i % nl_;
      // printf("launch reduce at node %d \n", node_);
      // fflush(stdout);
      cilk_spawn_at(&refPtr[node_]) this->template internal_reduce<TagType>(
          b, e, i, m_reduce_size * m_policy_par_size, nl_);
    }
    cilk_sync;

    cilk_reducer_wrapper* pReducerHost =
        get_reducer<cilk_reducer_wrapper>(global_reducer, 0);
    for (int i = 1; i < nl_; i++) {
      cilk_reducer_wrapper* pReducerNode =
          get_reducer<cilk_reducer_wrapper>(global_reducer, i);
      reduction_type lRef;
 ...
 public:
  inline void execute() const {
    ...
    serial_resize_thread_team_data(m_reduce_size, team_reduce_size,
  ...
  ParallelReduce(
      ...
      : m_functor(arg_functor),
        ...
        global_reducer(
            mw_malloc2d(Kokkos::Experimental::EmuReplicatedSpace::
                            memory_zones(),  // one for each memory zone...
                        sizeof(cilk_reducer_wrapper))),
        local_reducer(mw_malloc2d(
            Kokkos::Experimental::EmuReplicatedSpace::memory_zones(),
            sizeof(typename cilk_reducer_wrapper::local_reducer_type))),
        working_ptr(mw_malloc2d(
            Kokkos::Experimental::EmuReplicatedSpace::memory_zones(),
  ...
  inline ParallelReduce(const FunctorType& arg_functor, Policy arg_policy,
      ...
      : m_functor(arg_functor),
        ...
        m_reduce_size(get_reduce_size(m_functor, m_reducer)),
        global_reducer(mw_malloc2d(
            Kokkos::Experimental::EmuReplicatedSpace::memory_zones(),
            sizeof(cilk_reducer_wrapper)))  // one for each memory zone...
        ,
        local_reducer(mw_malloc2d(
            Kokkos::Experimental::EmuReplicatedSpace::memory_zones(),
            sizeof(typename cilk_reducer_wrapper::local_reducer_type))),
