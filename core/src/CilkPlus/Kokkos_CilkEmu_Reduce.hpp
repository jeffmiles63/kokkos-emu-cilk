...
ReducerType* get_reducer(const void* ptr, int refId) {
  ReducerType* pRet = (ReducerType*)mw_arrayindex(
      (void*)ptr, refId,
...
template <class ReduceWrapper, class ReducerType, class WorkTagFwd>
struct CilkEmuReduceView<
    ReduceWrapper, ReducerType, WorkTagFwd,
 ...
 public:
  inline CilkEmuReduceView(const void* rp, int id, rd_value_type& val_)
      : val(val_) {}

  inline void identity(const void* rp, int id, rd_value_type* val) {
    ...
    // ptr, *val); fflush(stdout);
  }

  inline void join(const void* rp, int id, rd_value_type right) {
    ReduceWrapper* ptr = get_reducer<ReduceWrapper>(rp, id);
    if (ptr) {
      while (true) {
  ...
  inline static rd_value_type create(int id, rd_value_type val_) {
    return val_;
  }

  inline static void destroy(int id, rd_value_type val_) {}
};
...
struct CilkReduceContainer<
  ...
  inline void reduce(const void* rp, int id, rd_value_type* left,
                     rd_value_type const* right) {
...
struct CilkEmuReduceView<
 ...
 public:
  ...
  inline static rd_value_type* create(const void* rp, int id,
                                      rd_value_type* val_) {
  ...
  inline static void destroy(const void* rp, int id, rd_value_type* val_) {
...
class NodeletReducer : public cilk::reducer<Mon> {
 ...
 public:
  NodeletReducer(const void* rp_, int id, const T* initVal)
      : m_id(id), myElt(*initVal), myView(rp_, id, *initVal) {}
  NodeletReducer(const void* rp_, int id, T initVal)
      : m_id(id), myElt(initVal), myView(rp_, id, myElt) {
    // printf("NR: %d - %d \n", id, myElt);
    // fflush(stdout);
  }
  ...
  virtual View* view() { return (&myView); }
};  // class
...
template <typename F>
struct lambda_only<F,
                   typename std::is_same<typename F::value_type, void>::type> {
  using type = typename F::value_type;
};

template <typename ReducerType, class Functor, class defaultType,
          class WorkTagFwd, class T = void>
struct kokkos_cilk_reducer;

template <typename ReducerType, class Functor, class defaultType,
          class WorkTagFwd>
struct kokkos_cilk_reducer<
    ReducerType, Functor, defaultType, WorkTagFwd,
    typename std::enable_if<Kokkos::is_reducer_type<ReducerType>::value ||
                            Kokkos::is_view<ReducerType>::value>::type> {
  typedef Kokkos::Impl::if_c<
      std::is_same<typename lambda_only<Functor>::type, void>::value,
      ReducerType, Functor>
      ReducerTypeCond;
  ...
  typedef Kokkos::Impl::FunctorValueInit<
      typename Kokkos::Impl::if_c<Kokkos::is_view<ReducerType>::value, Functor,
                                  ReducerType>::type,
      WorkTagFwd>
      ValueInit;
  ...
  kokkos_cilk_reducer(const void* gr_, const int id, const ReducerType& f_,
                      const size_t l_alloc_bytes, void* ptr_reducer)
      : m_id(id),
        f(f_),
        r(f_),
...
struct kokkos_cilk_reducer<
  ...
  typedef CilkReduceContainer<kokkos_cilk_reducer,
                              Kokkos::Experimental::Sum<defaultType>, void>
      reduce_container;
  ...
  kokkos_cilk_reducer(const void* gr_, const int id, const Functor& f_,
                      const size_t l_alloc_bytes, void* ptr_reducer)
      : m_id(id),
        local_value(0),
        f(f_),
        ...
        local_reducer(reinterpret_cast<local_reducer_type*>(ptr_reducer)),
        global_reducer(gr_) {
    // printf("constructing default scalar reducer (sum), size = %d , addr =
  ...
  void update_value(defaultType& ret) { ret = local_reducer->get_value(); }
...
struct kokkos_cilk_reducer<
  ...
  typedef CilkReduceContainer<kokkos_cilk_reducer, Functor, WorkTagFwd>
      reduce_container;
  typedef cilk::reducer<reduce_container> local_reducer_type;
  ...
  //       f = rhs.f ; alloc_bytes = rhs.alloc_bytes ; lVal = rhs.lVal ;
  //       local_reducer = rhs.local_reducer ; return *this ; }
  ...
  kokkos_cilk_reducer(const void* gr_, const int id, const Functor& f_,
    ...
    this->lVal     = (typename reduce_container::rd_value_type*)mw_localmalloc(
        l_alloc_bytes, (void*)myRed);
...
struct kokkos_cilk_reducer<
    ...
    typename std::enable_if<
        ...
        (!std::is_array<typename Functor::value_type>::value &&
         !std::is_pointer<typename Functor::value_type>::value)>::type> {
  typedef CilkReduceContainer<kokkos_cilk_reducer, ReducerType, WorkTagFwd>
      reduce_container;
  typedef cilk::reducer<reduce_container> local_reducer_type;
  typedef Functor ReducerTypeFwd;
  typedef Kokkos::Impl::FunctorValueJoin<Functor, WorkTagFwd> ValueJoin;
