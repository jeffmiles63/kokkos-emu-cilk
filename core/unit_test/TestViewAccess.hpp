...
template <class Scalar>
struct ConFunctor {
  typedef Kokkos::View<TestContainer<Scalar>*,
...
void TestViewAccess(int N) {
  ...
  Kokkos::parallel_for(
      N, KOKKOS_LAMBDA(int i) { hs_view(i) = i * 3; });
  Kokkos::fence();
  for (int i = 0; i < N; i++) {
  ...
  Kokkos::parallel_reduce(
      N,
      KOKKOS_LAMBDA(int i, Scalar& update) {
  ...
  //   long * refPtr = Kokkos::Experimental::EmuReplicatedSpace::getRefAddr();
     for (int i = 0; i <
  Kokkos::Experimental::EmuReplicatedSpace::memory_zones(); i++) {
        MIGRATE(&refPtr[i]);
  //      Kokkos::View< Scalar*, Kokkos::Experimental::EmuLocalSpace >
  local_space_view( Kokkos::ViewAllocateWithoutInitializing("local"), N );
        Kokkos::View< Scalar*, Kokkos::Experimental::EmuLocalSpace >
  local_space_view( "local", N );
  ...
  */
  ...
  {
    Kokkos::View<Scalar*, Kokkos::Experimental::EmuReplicatedSpace>
        replicated_space_view("replicated", N);
  ...
  Kokkos::fence();
  {
    Kokkos::View<Scalar*, Kokkos::HostSpace,
                 Kokkos::MemoryTraits<Kokkos::ForceRemote> >
        global_space_view("global", N);
    //      printf ("global view size is %ld \n", (unsigned
    //      long)sizeof(global_space_view)); Kokkos::View< Scalar*,
    //      Kokkos::HostSpace, Kokkos::MemoryTraits<0> > global_space_view(
    //      "global", N );
  ...
  for (int r = 0; r < 1; r++) {
    Kokkos::View<Scalar*, Kokkos::Experimental::EmuStridedSpace>
        strided_space_view("strided", N);
    //      printf ("strided view size is %ld \n", (unsigned
    //      long)sizeof(strided_space_view)); fflush(stdout);
  ...
  {
    Kokkos::View<Scalar**, Kokkos::Experimental::EmuStridedSpace>
        strided_space_view("2D strided", 8, N);
    //      printf ("strided view size is %ld \n", (unsigned
    //      long)sizeof(strided_space_view)); fflush(stdout);
  ...
  for (int r = 0; r < 1; r++) {
    Kokkos::View<TestContainer<long>*, Kokkos::Experimental::EmuStridedSpace>
        strided_space_view("strided", N);

    // have to initialize it first (view doesn't do that automatically right
    // now.)
    printf("initializing container view: %d \n", N);
    fflush(stdout);

    // void* test_data = (void*)mw_malloc2d(NODELETS(), block_size *
    // size_of_type);
    ConFunctor<long> cf(strided_space_view);
    // ConFunctor cf(test_data, block_size);
    Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::Experimental::CilkPlus,
                                             ConFunctor<long>::TagInit>(0, N),
                         cf);
